...
class Dali4Net {
    ...
    async startSearch(bus){
        ...
        if(this.bus[bus] && this.bus[bus].status) {
            ...
            for(const i in daligethex) {
                ...
                const message = this.getMessageGet(this.bus[bus].address, transactionId, lampId);
                ...
                if(exists){
                    ...
                    this.result[daliName[i]] = {
                        name: daliName[i],
                        value: Math.round((data.toJSON().data[14]/254)*100),
    ...
    lampExists(data,name) {
        ...
        if(data && (data[10] == 130 || data[10] == '0x82')) {
            this.adapter.log.info('find lamp ' + name);
            return true;
        }
        
        this.adapter.log.info('find not lamp ' + name);
        return false;
    }
...
////////////////////////////////get State Dali Bus/////////////////////////////////////////////////
    ...
    destroy() {
        clearInterval(this.adapter.schedule);
        this.client.destroy();
    }

    async counterObjects() {

        this.adapter.log.debug('obj ' + this.result)
        ...
        for(const i in this.result){
            ...
            const message = this.getMessageGet(this.result[i].busadd, transactionId, this.result[i].address);
            ...
            if(this.result[i].level != newlevel) {  // Soll ungleich Ist
                this.adapter.log.debug('level change')
                this.result[i].level = newlevel;
                this.responseState(newlevel, this.result[i].name); 
            } else {
...
////////////////////////////////set State Dali Light/////////////////////////////////////////////////
    ...
    sendLampState(bus, value, name) {
        ...
        const message = this.getMessageSet(this.bus[bus].address, transactionId, lampId, lampLevel,name);
        this.adapter.log.debug(JSON.stringify(message));
    ...
    sendGroupState(bus, value, name){
        ...
        const message = this.getMessageSet(this.bus[bus].address, transactionId, groupId, groupLevel,name); 
        this.adapter.log.debug(JSON.stringify(message));  
    ...
    sendScene(bus, name) {
        ...
        const message = this.getMessageSet(this.bus[bus].address, transactionId, broadcastScene, sceneId, name); 
        this.adapter.log.debug(JSON.stringify(message));  
    ...
    sendBroadcast(bus, value){
        ...
        const message = this.getMessageSet(this.bus[bus].address, transactionId, broadcast, broadcastLevel, 'broadcast'); 
        this.adapter.log.debug(JSON.stringify(message));  
        
        this.client.write(message);
    }

    getMessageSet(busAddress, transactionId, daliAddress, level, name){        
    ...
    getlampId(name){
        ...
        this.adapter.log.debug("Parse " + number)
