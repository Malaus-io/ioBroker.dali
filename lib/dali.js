...
const getStates = {
    switch: {
        old: await device.getState(),
        new: await device.getNewState()
    },
    jalousie: {
        old: await device.getJalState(),
        new: await device.getNewJalState()
    },
    motion: {
        old: await device.getMotionState(),
        new: await device.getNewMotionState()
    }
}*/
...
const typeEDaliClass4 = {5: 'Dali CS Temp', 6: 'Dali CS Irda', 7: 'Dali CS LS'};
const typeEDaliClass5 = {9: 'Dali Touch', 4: 'Dali MC', 5: 'Dali Switch', 6: 'Dali MC+'}
const typeEDaliClass6 = {1: 'Dali Sequencer'};                         
//DAli CS temp = 5 class 4
//Dali Switch =  
...
const searchDevice = [
    ...
    'ea00', 'ea01', 'ea02', 'ea03', 'ea04', 'ea05', 'ea06', 'ea07', 'ea08', 'ea09', 'ea10', 'ea11', 'ea12', 'ea13', 'ea14', 'ea15', 'ea16', 'ea17', 'ea18', 'ea19', 
    'ea20', 'ea21', 'ea22', 'ea23', 'ea24', 'ea25', 'ea26', 'ea27', 'ea28', 'ea29', 'ea30', 'ea31', 'ea32', 'ea33', 'ea34', 'ea35', 'ea36', 'ea37', 'ea38', 'ea39', 
    'ea40', 'ea41', 'ea42', 'ea43', 'ea44', 'ea45', 'ea46', 'ea47', 'ea48', 'ea49', 'ea50', 'ea51', 'ea52', 'ea53', 'ea54', 'ea55', 'ea56', 'ea57', 'ea58', 'ea59', 
    'ea60', 'ea61', 'ea62', 'ea63'
]
const daliName = [
    'a00', 'a01', 'a02', 'a03', 'a04', 'a05', 'a06', 'a07', 'a08', 'a09', 'a10', 'a11', 'a12', 'a13', 'a14', 'a15', 'a16', 'a17', 'a18', 'a19', 
...
const daliGetHex = [
    ...
    '29', '2b', '2d', '2f', '31', '33', '35', '37', '39', '3b', '3d', '3f', '41', '43', '45', '47', '49', '4b', '4d', '4f', 
    '51', '53', '55', '57', '59', '5b', '5d', '5f', '61', '63', '65', '67', '69', '6b', '6d', '6f', '71', '73', '75', '77', 
    '79', '7b', '7d', '7f', 
...
const daliSetHex = [
    ...
    '28', '2a', '2c', '2e', '30', '32', '34', '36', '38', '3a', '3c', '3e', '40', '42', '44', '46', '48', '4a', '4c', '4e', 
    '50', '52', '54', '56', '58', '5a', '5c', '5e', '60', '62', '64', '66', '68', '6a', '6c', '6e', '70', '72', '74', '76', 
    '78', '7a', '7c', '7e', 
...
class Dali4Net {
            ...
            for(const i in searchDevice) {
                ...
                if (searchDevice[i].indexOf('e') === 0) {
                    ...
                    for(const id in daliClass) {
                        ...
                        if (daliClass[id] === daliClass[4]){

                            typeName = typeEDaliClass4[data[14]];

                        }
                        else if (daliClass[id] === daliClass[5]){

                            typeName = typeEDaliClass5[data[14]];

                        }else if (daliClass[id] === daliClass[6]){

                            typeName = typeEDaliClass6[data[14]];
                        ...
                        const exists = this.deviceExists(data, searchDevice[i]);

                        if(exists) {
 
                            await this.createClass (typeName, searchDevice[i], daliGetHex[i], this.bus[bus].address, this.bus[bus].name, daliSize[1], daliClass[id]);
                            this.adapter.log.info('find ' + searchDevice[i] + ' type: ' + typeName);
                        }
                    ...
                    const exists = this.deviceExists(data, searchDevice[i]);
                        ...
                        if(exists) {
                            ...
                            if (typeName === 'LED Modules'){

                                message = this.getMessage(this.bus[bus].address, transactionId, '0x' + daliGetHex[i], daliOperatingMode, daliSize[0], daliClass[0]);
                                this.adapter.log.debug(JSON.stringify(message));

                                data = await this.sendMessage(message);
                                this.adapter.log.debug('Data ' + JSON.stringify(data));

                                if(data[14] === 147 || data[14]=== 144){

                                    typeName = await this.getRGBMode(data[14], this.bus[bus].address, daliGetHex[i]);
                    ...
                    const exists = this.deviceExists(data, searchDevice[i]);
                        ...
                        if(exists) {
                            ...
                            await this.createClass (typeName, searchDevice[i], daliGetHex[i], this.bus[bus].address, this.bus[bus].name, daliSize[0], daliClass[0]);
                            this.adapter.log.info('find group ' + searchDevice[i]);
                        }
                }  
            }
    ...
    async createClass (typeName, searchDevice, daliGetHex, busAdd, busName, size, daliClass){
        ...
        try {
            ...
            this.adapter.log.info('info ' + typeName + ' ' + daliGetHex + ' ' + searchDevice + ' ' + size + ' ' + daliClass);
    ...
    async getRGBMode(value, bus, address){

        let message;
        let data;
        const obj = { 144: { run: 3, 
                               0: 'Dali 3ch LED DIMMER red', 
                               1: 'Dali 3ch LED DIMMER green',
                               2: 'Dali 3ch LED DIMMER blue'
                            }, 
                      147: { run: 2,
                                0: 'Dali 3ch LED DIMMER bright', 
                                1: 'Dali 3ch LED DIMMER color',
                                2: null
                            }
                    }
        const transactionId = this.transactionIdentifier();
        message = this.getMessage(bus, transactionId, '0xa3', '0x21', daliSize[0], daliClass[0]);
        this.adapter.log.debug(JSON.stringify(message));

        data = await this.sendMessage(message);

            for (let l = 0; l<obj[value].run; l++){
            message = this.getMessage(bus, transactionId,'0x' + address , '0xc5', daliSize[0], daliClass[0]);
            this.adapter.log.debug(JSON.stringify(message));

            data = await this.sendMessage(message);
            
                if (address === data[14].toString(16)){
...
////////////////////////////////get State Dali Bus/////////////////////////////////////////////////
    ...
    async counterObjects() {
        ...
        try {
            for(const i in this.devices) {
                ...
                const getStates = await device.getStates()
                this.adapter.log.debug('getStates ' + JSON.stringify(getStates))
                ...
                this.adapter.log.debug('levelodl ' + levelOld);
                    ...
                    if (levelOld != null){
                        ...
                        this.adapter.log.info('pos ' + pos);
                        
                        if(pos != null){

                            levelNew = this.getLevel(lampStates, pos);
                            this.adapter.log.info('newlevel ' + levelNew);
                        ...
                        if (levelNew != levelOld){
                            ...
                            const id = device.getLevelId();
                            this.adapter.log.debug('path ' + path + id);
                            this.responseState(levelNew, name, path + id);
                        }
                    }
                for (const d in getStates){

                    const oldState = getStates[d].old
                    this.adapter.log.debug('oldState ' + oldState);

                    if (oldState != null){

                        const newState = getStates[d].new
                        this.adapter.log.debug('newState ' + newState);

                        if(oldState != newState){

                            getStates[d].set(newState);
                            const path = device.getPath();
                            const id = getStates[d].name
                            this.adapter.log.debug('nameID ' + id)

                            this.adapter.log.debug('path ' + path + id);
                            this.responseState(newState, name, path + id);
                        }
                    }
                }
                ...
                const motionStateOld = await device.getMotionState();
                this.adapter.log.debug('motionStateOld ' + motionStateOld);
            
                    if (motionStateOld != null){
                        
                        const stateNew = await device.getNewMotionState();
                        this.adapter.log.debug('newstate ' + stateNew);

                        if (stateNew != motionStateOld){
    ...
    getLevel(data, pos = 14) {
        this.adapter.log.info('dataPOS ' + data.toJSON().data[pos])
        
        let l;
        if(data.toJSON().data[pos] === 253){

            l = 99;

        }else{

            l = Math.round((data.toJSON().data[pos]/254)*100);
    ...
    async sendLampState(bus, value, name, folder) {
        ...
        let query;
        let lampId;
this.adapter.log.info('value ' + value)
        if(name === folder){
            ...
            query = '0x' + value.toString(16);
            this.adapter.log.info('colorLevelhex ' + query);
            lampId = '0x' + daliSetHex[daliName.indexOf(folder)];

        }else{
        
            if(value != true){
                query = queryDali[name][value];
            }else{
                query = queryDali[name];
            }
            this.adapter.log.info('query ' + query);
            lampId = '0x' + daliGetHex[daliName.indexOf(folder)];
        }

        const transactionId = this.transactionIdentifier();
        const classSize = this.devices[folder].getClass();
        const size = this.devices[folder].getSize();
        this.adapter.log.info('lampId ' + lampId);        

        const message = this.getMessage(this.bus[bus].address, transactionId, lampId, query, size, classSize);
