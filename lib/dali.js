...
const type = ['Fluoresecent Lamps', 'Emergency lighting', 'Discharge Lamps', 'Low Voltage Halogen Lamps', 'Supply Voltage Regulator', 
...
class Dali4Net {
        ...
        if(this.bus[bus] && this.bus[bus].status) {
            ...
            for(const i in daliName) {
                ...
                if (daliName[i].indexOf('e') === 0){
                    message = this.getMessage(this.bus[bus].address, transactionId, '0x' + daliGetHex[i], '0xc0', daliSize[1], daliClass[1]);
                ...
                }else if (daliName[i].indexOf('a') === 0 || daliName[i].indexOf('g') === 0) {
                    message = this.getMessage(this.bus[bus].address, transactionId, '0x' + daliGetHex[i], queryLevel, daliSize[0], daliClass[0]);
    ...
    async moreInformation(bus, address){
        ...
        for(const i in query){
            ...
            const message = this.getMessage(bus, transactionId, '0x' + address, query[i], daliSize[0], daliClass[0]);
            this.adapter.log.debug(JSON.stringify(message));
...
////////////////////////////////get State Dali Bus/////////////////////////////////////////////////
    ...
    async counterObjects() {
        ...
        this.newlevel = 0;
        this.adapter.log.info('obj ' + JSON.stringify(this.result))
        ...
        for(const i in this.result) {
            ...
            if (name.indexOf('e')===0){
                ...
                const message = this.getMessage(this.result[i].busadd, transactionId, '0x' + '05', '0xc0', daliSize[1], daliClass[1]);
                this.adapter.log.info('create Message group '+ JSON.stringify(message))
                ...
                if(name.indexOf('a')===0) {
                    ...
                    const message = this.getMessage(this.result[i].busadd, transactionId, '0x' + this.result[i].address, query[4], daliSize[0], daliClass[0]);
                    this.adapter.log.info('create Message group '+ JSON.stringify(message))
