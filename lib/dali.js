...
class Dali4Net {
    ...
    async startSearch(bus) {
        ...
        this.adapter.log.info('device result' + this.devices)
        return this.devices;
        }
    }
    ...
    async createClass (typeName, searchDevice, daliGetHex, busAdd, busName, size, daliClass){
        ...
        try {
            ...
            this.adapter.log.info('type info' + typeName)
            this.adapter.log.info('dalihex ' + daliGetHex)
            this.adapter.log.info('i ' + searchDevice)
...
////////////////////////////////get State Dali Bus/////////////////////////////////////////////////
    ...
    async counterObjects() {
        ...
        for(const i in this.devices) {
            ...
            const name = this.devices[this.devices[i]].getName();
    
            const levelOld = await this.devices[this.devices[i]].getLevel();
            this.adapter.log.info('levelodl ' + levelOld);
       
                if (levelOld != null){
                    
                    let levelNew = 0;
                    const pos = this.devices[this.devices[i]].getLevelpos()
                    this.adapter.log.info('pos ' + pos)
                    
                    if(pos != null){

                        levelNew = this.getLevel(lampStates, this.devices[this.devices[i]].getLevelpos());
                        this.adapter.log.info('newlevel ' + levelNew)
    
                    }else{

                        levelNew = await this.devices[this.devices[i]].getNewLevel();
                        this.adapter.log.info('newLevel ' + levelNew)
                    ...
                    if (levelNew != levelOld){
                        ...
                        this.devices[this.devices[i]].setNewLevel(levelNew);
                        const path = this.devices[this.devices[i]].getPath();
                        this.adapter.log.info('path ' + path + name)
                        this.responseState(levelNew, name, path + name);
                    }
                }
            
            const stateOld = await this.devices[this.devices[i]].getState();
            this.adapter.log.info('stateOld ' + stateOld);
        
                if (stateOld != null){
                    
                    const stateNew = await this.devices[this.devices[i]].getNewState();
                    this.adapter.log.info('newstate ' + stateNew)
                    if (stateNew != stateOld){

                        this.devices[this.devices[i]].setNewState(stateNew);
                        const path = this.devices[this.devices[i]].getPath();
