...
const queryLevel = 0x99//0x90; //Actual level
...
class Dali4Net {
        ...
        if(this.bus[bus] && this.bus[bus].status) {
            ...
            for(const i in daliName) {
                ...
                if (daliName[i].indexOf('e') === 0){
                    message = this.getMessage(this.bus[bus].address, transactionId, '0x' + daliGetHex[i], queryEDali[0], daliSize[1], daliClass[1]);
                    ...
                    if(exists) {
                        ...
                        const info = await this.moreInformationEDali(this.bus[bus].address, daliGetHex[i])
                        this.result[daliName[i]] = {
                                                        name: daliName[i],
                                                        value: info[0],  //this.getLevel(data),
                                                        busname: this.bus[bus].name,
                                                        busadd: this.bus[bus].address,
                                                        address: daliGetHex[i],
                                                        mode: accessMode[info[1]]
                    ...
                    if(exists) {
                        //this.devices.searchDevice(data[14], bus, i)
                        //this.adapter.info( 'info' + this.result[i])
    ...
    async moreInformation(bus, address){
        ...
        for(const i in queryDali){

            this.adapter.log.debug('query ' + queryDali[i])
            const transactionId = this.transactionIdentifier();

            const message = this.getMessage(bus, transactionId, '0x' + address, queryDali[i], daliSize[0], daliClass[0]);
            this.adapter.log.debug(JSON.stringify(message));
