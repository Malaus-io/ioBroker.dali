...
class Dali4Net {
                    ...
                    const exists = this.deviceExists(data, searchDevice[i], typeName);
                        ...
                        if(exists) {
                            ...
                            if (typeName === 'LED Modules'){
                                ...
                                data = await this.sendMessage(message);
                                this.adapter.log.debug('Data ' + JSON.stringify(data));
                                this.adapter.log.info('a' + a)

                                if(data[14] === 147){

                                    
                                    message = this.getMessage(this.bus[bus].address, transactionId, '0xa3', '0x21', daliSize[0], daliClass[0]);
                                    this.adapter.log.info(JSON.stringify(message));

                                    data = await this.sendMessage(message);

                                    for (let l = 0; l<2; l++){
                                    message = this.getMessage(this.bus[bus].address, transactionId,'0x' + daliGetHex[i] , '0xc5', daliSize[0], daliClass[0]);
                                    this.adapter.log.info(JSON.stringify(message));

                                    data = await this.sendMessage(message);
                                    this.adapter.log.info('l ' + l + ' test ' + daliGetHex.indexOf(data[14]))
                                    this.adapter.log.info('data ' + JSON.stringify(data))
                                    this.adapter.log.info('datahex ' + data[14].toString(16))
                                        this.adapter.log.info('daligethex ' + daliGetHex[i]);
                                        if (daliGetHex[i] === data[14].toString(16)){

                                            if(l === 0){

                                                typeName = 'Dali 3ch LED DIMMER bright';

                                            }else{

                                                typeName = 'Dali 3ch LED DIMMER color';
                                    ...
                                    this.adapter.log.info('datahex ' + data[14].toString(16))
                                        ...
                                        if (daliGetHex[i] === data[14].toString(16)){
                                            ...
                                            if(l === 0){
                                                ...
                                                typeName = 'Dali 3ch LED DIMMER red';

                                            }else if (l === 1){

                                                typeName = 'Dali 3ch LED DIMMER green';

                                            }else{

                                                typeName = 'Dali 3ch LED DIMMER blue';
    ...
    async createClass (typeName, searchDevice, daliGetHex, busAdd, busName, size, daliClass){
        ...
        try {
            ...
            this.adapter.log.info('info ' + typeName + ' ' + daliGetHex + ' ' + searchDevice);
            ...
            const level = await this.devices[searchDevice].getLevel();
            this.adapter.log.info('level '+ level)
