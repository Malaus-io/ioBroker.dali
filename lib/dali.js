...
const queryLevel = 0xa0; //Actual level
//const query [device type, groups 0,7 , groups 8,15, physical min level]
const query = ['0x99', '0xc0', '0xc1', '0x9a']
    ...
    async startSearch(bus) {
        ...
        if(this.bus[bus] && this.bus[bus].status) {
            ...
            for(const i in daliGetHex) {
                ...
                const message = this.getMessage(this.bus[bus].address, transactionId, '0x' + daliGetHex[i], queryLevel);
    ...
    async counterObjects() {
        ...
        for(const i in this.result) {
            ...
            if(name.indexOf('a')===0) {
                ...
                const message = this.getMessage(this.result[i].busadd, transactionId, '0x' + this.result[i].address, queryLevel);
