    ...
    async infoProcess (bus, address, name){
        ...
        let common;
            ...
            switch(i) {
                ...
                case 'level':
                    data =  await this.getInfo(bus, address, lib.device[name].info[i][0], lib.device[name].size, lib.device[name].class);
                    result[i] = [Math.round((data[14]/254)*100), lib.device[name].info[i][1]];
                    break;
                
                case 'group07':
                    this.adapter.log.info('group07 ' + lib.device[name].info[i])
                    data =  await this.getInfo(bus, address, lib.device[name].info[i], lib.device[name].size, lib.device[name].class);
